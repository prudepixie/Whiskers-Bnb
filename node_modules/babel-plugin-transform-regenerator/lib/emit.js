...
function Emitter(contextId) {
  ...
  // A sparse array whose keys correspond to locations in this.reservation
...
// Offsets into this.reservation that could be used as targets for branches or
...
Ep.mark = function (loc) {
  t.assertLiteral(loc);
  var index = this.reservation.length;
  if (loc.value === -1) {
    loc.value = index;
  } else {
...
// Turns this.reservation into a loop of the form
//
...
//   }
//
// Each marked location in this.reservation will correspond to one generated
// case statement.
Ep.getDispatchLoop = function () {
  ...
  self.reservation.forEach(function (stmt, i) {
  ...
  // Now that we know how many statements there will be in this.reservation,
  // we can finally resolve this.finalLoc.value.
  this.finalLoc.value = this.reservation.length;
...
// Not all offsets into emitter.reservation are potential jump targets. For
...
Ep.getUnmarkedCurrentLoc = function () {
  return t.numericLiteral(this.reservation.length);
};
...
Ep.updateContextPrevLoc = function (loc) {
  if (loc) {
    ...
    if (loc.value === -1) {
      ...
      // to the current this.reservation.length.
      loc.value = this.reservation.length;
    } else {
      // Otherwise assert that the location matches the current offset.
      _assert2.default.strictEqual(loc.value, this.reservation.length);
    }
  } else {
